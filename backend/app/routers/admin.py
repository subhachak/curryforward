...
def list_all_recipes(db: Session = Depends(get_db), role: str = Depends(require_admin)):
    ...
    for r in rows:
        ...
        result.append({
            ...
            "intro": r.intro,
            "completeness": {
                "ingredients": bool(r.components),
                "steps": bool(r.steps),
                "intro": bool(r.intro),
                "timing": r.prep_time_minutes is not None or r.cook_time_minutes is not None,
                "image": bool(r.hero_image_url),
                "category": bool(r.category),
            },
